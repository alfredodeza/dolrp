...
DreamObjectsPublishServiceProvider.lua
Publish-specific portions of Lightroom DreamObjects uploader
...
--- The <i>service definition script</i> for a publish service provider associates
 ...
 -- <p>The <code>DreamObjectsPublishSupport.lua</code> file of the DreamObjects sample plug-in provides
 ...
 -- 	<code>DreamObjectsExportServiceProvider.lua</code> file.</p>
...
publishServiceProvider.small_icon = 'dreamhost-logo.png'
...
publishServiceProvider.titleForPublishedCollection = LOC "$$$/DreamObjects/TitleForPublishedCollection=Photoset"
...
publishServiceProvider.titleForPublishedCollection_standalone = LOC "$$$/DreamObjects/TitleForPublishedCollection/Standalone=Photoset"
...
-- publishServiceProvider.titleForPublishedCollectionSet = "(something)" -- not used for DreamObjects plug-in
...
--publishServiceProvider.titleForPublishedCollectionSet_standalone = "(something)" -- not used for DreamObjects plug-in
...
publishServiceProvider.titleForPublishedSmartCollection = LOC "$$$/DreamObjects/TitleForPublishedSmartCollection=Smart Photoset"
...
publishServiceProvider.titleForPublishedSmartCollection_standalone = LOC "$$$/DreamObjects/TitleForPublishedSmartCollection/Standalone=Smart Photoset"
...
function publishServiceProvider.getCollectionBehaviorInfo( publishSettings )
	...
	return {
		defaultCollectionName = LOC "$$$/DreamObjects/DefaultCollectionName/Photostream=Photostream",
		defaultCollectionCanBeDeleted = false,
		canAddCollection = true,
		maxCollectionSetDepth = 0,
			-- Collection sets are not supported through the DreamObjects sample plug-in.
	}
...
publishServiceProvider.titleForGoToPublishedCollection = LOC "$$$/DreamObjects/TitleForGoToPublishedCollection=Show in DreamObjects"
...
publishServiceProvider.titleForGoToPublishedPhoto = LOC "$$$/DreamObjects/TitleForGoToPublishedCollection=Show in DreamObjects"
...
function publishServiceProvider.metadataThatTriggersRepublish( publishSettings )
	...
	return {
		...
		-- also (not used by DreamObjects sample plug-in):
			-- customMetadata = true,
...
--[[ Not used for DreamObjects plug-in. This is an example of how this function might work.
...
--- (optional) This plug-in defined callback function is called when new or updated
 ...
 -- the user-specified sort order should be followed as-is or reversed. The DreamObjects
 -- sample plug-in uses this to reverse the order on the Photostream so that photos
 -- appear in the DreamObjects web interface in the same sequence as they are shown in the
 -- library grid.
 -- <p>This is not a blocking call. It is called from within a task created
 -- using the <a href="LrTasks.html"><code>LrTasks</code></a> namespace. In most
 -- cases, you should not need to start your own task within this function.</p>
...
function publishServiceProvider.imposeSortOrderOnPublishedCollection( publishSettings, info, remoteIdSequence )
	...
	if photosetId then
		...
		local existingPhotoSequence = DreamObjectsAPI.listPhotosFromPhotoset( publishSettings, { photosetId = photosetId } )
		...
		if existingPhotoSequence and existingPhotoSequence.primary then
			DreamObjectsAPI.setPhotosetSequence( publishSettings, {
...
-- publishServiceProvider.disableRenamePublishedCollection = true -- not used for DreamObjects sample plug-in
...
-- publishServiceProvider.disableRenamePublishedCollectionSet = true -- not used for DreamObjects sample plug-in
...
function publishServiceProvider.renamePublishedCollection( publishSettings, info )
	...
	if info.remoteId then
		...
		DreamObjectsAPI.createOrUpdatePhotoset( publishSettings, {
...
function publishServiceProvider.deletePublishedCollection( publishSettings, info )
	...
	import 'LrFunctionContext'.callWithContext( 'publishServiceProvider.deletePublishedCollection', function( context )
		...
		local progressScope = LrDialogs.showModalProgressDialog {
							title = LOC( "$$$/DreamObjects/DeletingCollectionAndContents=Deleting photoset ^[^1^]", info.name ),
		...
		if info and info.remoteId then
			...
			DreamObjectsAPI.deletePhotoset( publishSettings, {
								photosetId = info.remoteId,
								suppressError = true,
									-- DreamObjects has probably already deleted the photoset
...
publishServiceProvider.titleForPhotoRating = LOC "$$$/DreamObjects/TitleForPhotoRating=Favorite Count"
...
function publishServiceProvider.canAddCommentsToService( publishSettings )
	...
	return DreamObjectsAPI.testDreamObjectsConnection( publishSettings )
...
function publishServiceProvider.addCommentToPublishedPhoto( publishSettings, remotePhotoId, commentText )
	...
	local success = DreamObjectsAPI.addComment( publishSettings, {
...
DreamObjectsPublishSupport = publishServiceProvider
